...
// Form contains fields from html page as well as any associated errors
type Form struct {
	url.Values
	Errors formErrors
}
...
func (f *Form) MaxLength(field string, n int) *Form {
	...
	if utf8.RuneCountInString(value) > n {
		f.Errors.Add(field, fmt.Sprintf("This field %s is too long. Max of %d characters.", field, n))
	}
	return f
}
...
func (f *Form) MinLength(field string, n int) *Form {
	...
	if utf8.RuneCountInString(value) < n {
		f.Errors.Add(field, fmt.Sprintf("This field %s is too short. Min of %d characters.", field, n))
	}
	return f
}
